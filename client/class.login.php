...
class logmein {
	...
	var $database_logon = 'Stores';		//Database NAME
	var $username_logon = 'ccis_user';		//Database USERNAME
	var $password_logon = 'ccis_password';		//Database PASSWORD
	
	//table fields
	var $user_table = 'List';			//Users table name
	var $user_column = 'name';			//USERNAME column (value MUST be valid email)
	var $pass_column = 'password';			//PASSWORD column
	var $user_level = 'sla';			//(optional) userlevel column
	...
	function login($table, $username, $password){
		...
		if($row != "Error"){
			if($row[$this->user_column] !="" && $row[$this->pass_column] !=""){
				...
				$_SESSION['login'] = $row[$this->user_column];
	...
	function loginform($formname, $formclass, $formaction){
		//conect to DB
		global $text_login;
		global $text_login_name;
		global $text_login_password;
		$this->dbconnect();
		echo'<div class="inner"><form name="'.$formname.'" method="post" id="'.$formname.'" enctype="application/x-www-form-urlencoded" action="'.$formaction.'">
				<label for="username">'.$text_login_name.'</label><br>
				<input class="rounded-textbox-login" name="username" id="username" type="text"><br><br>
				<label class="innerLabel" for="password">'.$text_login_password.'</label><br>
				<input class="rounded-textbox-login" name="password" id="password" type="password"><br>
				<input name="action" id="action" value="login" type="hidden"><br>
				<input class="buttonLogin" name="submit" id="submit" value="'.$text_login.'" type="submit">
			</form></div>';
	}
	//reset password form
